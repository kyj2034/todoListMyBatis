...
export default {
  ...
  methods: {
    ...
    updTodo() {
      if (this.updTodoVal !== '') {
        const val = this.updTodoVal && this.updTodoVal.trim();
        const item = { key: this.getTodoList[this.getChkUpdIdx].key,
          val,
          finish: false };
    ...
    delTodo() {
      const delItems = [];
      console.debug(this.getChkDelIdx);
      // eslint-disable-next-line no-restricted-syntax
      for (const idx of this.getChkDelIdx) {
        delItems.push(this.getTodoList[idx].key);
      }
      this.removeTodoAct(delItems);
