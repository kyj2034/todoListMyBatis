<template>
  <section>
    ...
    >
      ...
      <li
        v-for="({id, todo, finish, key}, idx) in todoList"
        class="shadow"
        :key="key"
      ...
      >
        <label class="checkbox">
          <input
            type="checkbox"
            id="chkBox"
            @change="finTodo({id, todo, finish, key, idx})"
          />
...
export default {
  ...
  methods: {
    ...
    finTodo({ id, todo, finish, idx }) {
      const finItem = { id, todo, finish: !finish, idx };
      this.finishTodoAct(finItem);
    },
  },
};
</script>
