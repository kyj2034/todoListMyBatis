...
export default new Vuex.Store({
  ...
  actions: {
     ...
     */
      const data = async addTodoAct(state, payload) { // action에서는 데이터를 푸시하고 삭제하는 기능만!!
        await axios.post('/todolist', payload).then(
          (response) => {
            console.debug(response);
            return response.data.data;
          },
        );
         state.todolist.push(response.data.data);
      },
       ...
       * @param {*} state
       */
      async clearAllAct(state) {
        // localStorage.clear() 는 위험함. 키 값으로 로컬 스토리지의 투두리스트 삭제
  
        await axios.delete('/todolist/all').then(
          (response) => {
       ...
       */
      async removeTodoAct(state, payload) {
       ...
       */
      async updateTodoAct(state, payload) {
        // 수정 axios 메소드로 제작
        const alert = await axios.put(`/todolist/${payload.key}`, payload).then(
          (response) => {
            console.debug(response);
            state.todoList[state.chkUpdIdx].todo = response.data.data.todo;
            return response;
          },
        ).catch((e) => {
          console.debug(e);
