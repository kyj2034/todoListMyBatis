...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    /* Getter 기능을 하는 mutation */
    removeListData : (state,getters) =>{
      const list = getters.getTodoList;
      const delList = getters.getChkDelIdx;
    ...
    saveListData : (state,getters,payload) => {
      const list = getters.getTodoList;
      list.push(payload);
      state.todoList = list;
    },
    updateListDataTodo : (state,getters,payload) => {
      const list = getters.getTodoList;
      list[getters.getChkUpdIdx].todo = payload;
      state.todoList = list;
    },
    updateListDataFin : (state, getters) =>  {
      const list = getters.getTodoList;
      list[getters.getChkUpdIdx].finish = !list[getters.getChkUpdIdx].finish;
      state.todoList = list;
    }
     ...
     */
       ...
       async addTodoAct({commit}, payload) { // action에서는 데이터를 푸시하고 삭제하는 기능만!!
          ...
          (response) => {
            ...
            // console.debug(list)
            // const list = dispatch('loadTodoAct');
            // list.push(response.data.data);
            commit('saveListData', response.data.data);
            return response;
          },).catch((e) => {
            return e;
          });
      ...
      async removeTodoAct({commit}, payload) {
          ...
          (response) => {
            console.debug(response);
            // const list = commit('getTodoList');
            // const delList = commit('getChkDelIdx');
            // console.debug(delList);
            // for(const idx of delList){
            //   list.splice(idx, 1);
            // }
            // const list = dispatch('setRemoveAct');
      ...
      async updateTodoAct({commit}, payload) {
        ...
        const update = await axios.put(`/todolist/${payload.key}`, payload).then(
          (response) => {
            ...
            //const list = dispatch('loadTodoAct');
            commit('updateListDataTodo', response.data.data.todo);
            return response;
          },
        ).catch((e) => {
          console.debug(e);
       ...
       */
        // 부울 메시지 받기 
      async finishTodoAct({commit}, { key, todo, finish}) {
